...
fn main() {
    ...
    let J = H * &Scalar::from(scalar2);
    let J_bt = EdwardsBasepointTable::create(&J);
    ...
    let r2 = Scalar::from(14u64);
    let v2 = Scalar::from(10u64);
    let t2 = Scalar::from(20u64);
    let test = Scalar::from(0u64);
    let rG_2 = EdwardsBasepointTable::basepoint_mul(&G_bt, &r2);
    let rG_4 = EdwardsBasepointTable::basepoint_mul(&H_bt, &v2);
    let vH_2 = EdwardsBasepointTable::basepoint_mul(&J_bt, &t2);
    let zero = EdwardsBasepointTable::basepoint_mul(&H_bt, &test);
    let zero2 = EdwardsBasepointTable::basepoint_mul(&G_bt, &test);

    // //TxIN3 from the article example (a=10, r=43): 43*G, 10*H
    let r3 = Scalar::from(43u64);
    let v3 = Scalar::from(10u64);
    let rG_3 = EdwardsBasepointTable::basepoint_mul(&H_bt, &r3);
    let vH_3 = EdwardsBasepointTable::basepoint_mul(&H_bt, &v3);

    //calculate pedersen EC commitments for the TxIN's
    //TxIN1: c=14*G+10*H
    let c1 = rG_1 + vH_1; // G(14) + H(10)
    //TxIN2: c=85*G+30*H
    let c2 = rG_2 + vH_2 + rG_4; // G(14) + H(10) + J(20)
    //TxIN3: c=43*G+10*H
    let c3 = zero + zero2 + vH_2; // G(0) + H(0) + J(20);

    let combined = rG_1 + vH_1 + rG_2 + vH_2 + rG_3 + vH_3;
    println!(
        "Do properties hold: {:?}",
        (c2 - c1).compress() == c3.compress()
    );
